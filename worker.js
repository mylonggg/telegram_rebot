...
async function getDatabaseStats(DB) {
  try {
    ...
    const stats = {
      ...
      msg_map_count: await DB.prepare('SELECT COUNT(*) as count FROM msg_map').first().then(r => r.count),
...
async function handleFraudUsersAPI(request, DB, password) {
  ...
  try {
    ...
    let query = `SELECT f.user_id, f.created_at, b.is_blocked 
                 FROM fraud_users f
                 LEFT JOIN blocked_users b ON f.user_id = b.chat_id`;
    ...
    const usersResult = await DB.prepare(query).bind(limit, offset).all();
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            <div class="col-md-12">
                ...
                <p class="text-muted">管理被标记为诈骗的用户列表，支持手动添加、搜索和删除操作</p>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            <span class="badge bg-light text-dark" id="filtered-count">0 个可见</span>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <p><strong>API端点:</strong> <code id="api-endpoint">/db-stats</code></p>
                                <p><strong>当前时间:</strong> <span id="current-time">-</span></p>
                            </div>
                            <div class="col-md-6">
                                <p><strong>页面版本:</strong> 1.0.0</p>
                                <p><strong>数据更新方式:</strong> 手动刷新 / 自动更新</p>
                                <p><strong>支持的操作:</strong> 查看、搜索、添加、删除</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadUsers(page = 1) {
            ...
            try {
                ...
                let url = \`/admin-api/fraud-users?page=\${page}&limit=\${itemsPerPage}&password=\${encodeURIComponent(adminPassword)}\`;
                if (search) {
